...
class svg(object):
    ...
    def svg_to_robotpath(self,paths,T,B):
        #this function is to transform the xy coordinates of the svg path to the 3D coordinates of the robot path
        final_path = []
...
def transformation(x_max,y_max,LB,TL,TR):
    #finds Transformation matrix and b
    b=LB
    t00 = (TL[0] - b[0]) / x_max
    t10 = (TL[1] - b[1]) / x_max
    t20 = (TL[2] - b[2]) / x_max
    t01 = (TR[0] - t00 * x_max - b[0]) / y_max
    t11 = (TR[1] - t10 * x_max - b[1]) / y_max
    t21 = (TR[2] - t20 * x_max - b[2]) / y_max
    
    #converts to correct matrix dimensions
    b=np.array([[LB[0]], [LB[1]],[LB[2]]])
...
if __name__ == '__main__':
    ...
    robot = dorna()
    print("connecting")
    robot.connect("IP", 443)
    wait_id = 100
    print("done connecting")
    

    #set the width and length of image wanted
    width = 200    
    length = 150
    ...
    #LB= np.array([25,20,10])
    #M= np.array([25,40,10])
    #TR= np.array([50,80,10])
    error = 0
    print("TL",TL)
    print("M",M)
    print("BR",BR)
    ...
    T, B= transformation(width,length,TL,TR,BR)

    #perpendicular vector in unit vector form
    cp = perpendicularvector(TL,M,BR)
    print(cp)
    #a=np.array([[1,1],[1,1],[1,1]],dtype='f')
    #b=np.array([[1],[1],[1]],dtype='f')
    #cp=[[0],[0],[1]]
    ...
    while stop:
        for cmd in cmds:
            for c in cmd:
                ...
                #print("length of command list",command_list)
                #print("msg",command_list.msg)
                #print("command list",len(command_list))
                #print("cmds_length",cmds_length[i])
                if len(command_list)==cmds_length[i]:
                    
                    last_continous_point = command_list.pop()
                    #print("command",command.msg)
