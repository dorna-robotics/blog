...
|Items|Links|
|---|----|
|Two 3D printed parts|[link](https://github.com/dorna-robotics/dorna2_otto/tree/main/drawing/STL%20files)|
...
Here we use Python for programming the robot. You need to download [Python](https://www.python.org/downloads/) and [Dorna 2 Python API](https://doc.dorna.ai/docs/api/python/manual/) if you haven't already done so. The line below allows you to connect to your Dorna 2. Replace the `"robot_ip_address"` based on your robot ip address, [for more information on the API](https://doc.dorna.ai/docs/api/python/manual/).
```python
...
The T stands for transformation and can be solved using the 3 corners of the plane. T is a 3x2 matrix. B is a 3x1 matrix. B is the starting position's coordinates. 
...
cmds, cmds_length = svg(10).gen(‘filename.svg', width, length, 0, 0, a, b, cp, scale, velocity, acceleration, jerk, corner)
```
The last few lines of the code takes the list of commands created from the the function above and sends it to the robot to perform. The robot will close the WebSocket connection and stop moving when the image is done. [For more information on robot commands](https://doc.dorna.ai/docs/cmd/intro/).
```python
 command_list=[]
 i=0
 stop=True
